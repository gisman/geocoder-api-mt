...
참조 데이터는 {config.CODE_DATA_DIR}/TN_SPRD_RDNM.txt 이다. 다운로드: https://business.juso.go.kr/addrlink/attrbDBDwld/attrbDBDwldList.do?cPath=99MD&menu=도로명
...
class RoadMatcher:
    # {config.CODE_DATA_DIR}/TN_SPRD_RDNM.txt 파일을 읽어와서 dict로 반환하는 클래스
    fieldnames = [
        "시군구코드",
        "도로명번호",
        "읍면동일련번호",
    ...
    def __init__(self):
        filename = f"{config.CODE_DATA_DIR}/TN_SPRD_RDNM.txt"
        self.road_dict = {}
