...
class Tokenizer:
    ...
    Attributes:
        ...
        re_tokenize_ri_n_err (re.Pattern): 리명과 번지 띄어쓰기 오타를 추출하기 위한 정규 표현식.
        ...
        road_like_h4 (set): 도로명처럼 보이지만 행정동 또는 법정동명 리스트.
    ...
    def __init__(self):
        ...
        self.re_tokenize_ri_n_err = re.compile(r"(.+리)\d+(-\d+)?")  # 선유리396-22 오타
        ...
        self.re_bunji_n_번지 = re.compile(r"^산?\d+(번지)?$")  # 1(번지)
        ...
        self.hd2ch = set(load_list("hd2ch.txt"))
        ...
        self.numRoadPatterns = set(load_list("numRoadPatterns.txt"))
        self.road_like_h4 = set(load_list("road_like_h4.txt"))
    ...
    def __assumeTokenType(self, toks: Tokens, ambiguous_only=False):
        """
        ...
        for pos in range(len(toks)):
            ...
            if self.__isH1(
                tkn.val
            ):  # h1. "경기 광주시"의 광주시가 광주광역시가 되지 않도록 H1이 있는지 검사
                ...
                beforeComma
                and prevType in ("HEAD", TOKEN_H1, TOKEN_H23)
                and tkn.val in (self.road_like_h4)
            ):  # h4
                tkn.t = TOKEN_H4

            elif (  # 리 주소 추가. 2025.06.06
                tkn.val.endswith("리")
                and beforeComma
                and prevType in ("HEAD", TOKEN_H1, TOKEN_H23, TOKEN_H4)
            ):  # h5
                tkn.t = TOKEN_RI
                ...
                and prevType
                in (
                    "HEAD",  # 길 이름으로 시작하는 주소 (2025.05.31 추가)
                    TOKEN_H1,
                    TOKEN_H23,
                    TOKEN_H4,
                    TOKEN_RI,
            ...
            elif tkn.val == "산":  # 산 토큰은 무조건 TOKEN_SAN
                tkn.t = TOKEN_SAN
    ...
    def tokenize(self, address):
        ...
        if h2pos > -1 and toks[h2pos] in self.__h3:
            # 3차행정구역 검사
        ...
        if h4pos > 0 and toks.index(TOKEN_BLD) == h4pos + 1:
            ...
            m = self.re_tokenize_ri_n_err.match(
                bldTok.val
            )  # 리와 번지 뜨어쓰기 오류. 선유리396-22
            if m:
                toks.split(bldPos, len(m.group(1)), TOKEN_RI, TOKEN_BNG)
        ...
        self.__mergeTokenSequence(
            toks,
            [TOKEN_H1, TOKEN_H1, TOKEN_H4],
            [TOKEN_H1, TOKEN_H23, TOKEN_H4],
        )

        # 광주시 XXX동 -> 경기도 광주시
        h1pos = toks.index(TOKEN_H1)
        if h1pos == 0 and toks.get(h1pos).val.startswith("광주"):
            self.__mergeTokenSequence(
                toks,
                [TOKEN_H1, TOKEN_H4],
                [TOKEN_H23, TOKEN_H4],
        ...
        if toks.index(TOKEN_ROAD) > -1 and toks.index(TOKEN_BLDNO) < 0:
            self.__mergeTokenSequence(
                toks,
        ...
        self.__mergeTokenSequence(
            toks,
            [TOKEN_H23, TOKEN_UNKNOWN, TOKEN_FLOOR],
            [TOKEN_H23, TOKEN_BLD, TOKEN_FLOOR],
        )

        # H23-UNKNOWN
        self.__mergeTokenSequence(
            toks,
            [TOKEN_H23, TOKEN_UNKNOWN],
            [TOKEN_H23, TOKEN_BLD],
        )
        ...
        # (주)위드체인지. 건물명 주소인데 건물명으로 인식되지 않는 경우
        if (
            toks.index(TOKEN_BNG) < 0
            and toks.index(TOKEN_BNG) < 0
            and toks.index(TOKEN_BLD) < 0
        ):
            self.__mergeTokenSequence(
                toks,
                [TOKEN_EXTRA, TOKEN_UNKNOWN],
                [TOKEN_EXTRA, TOKEN_BLD],
            )

        if toks.index(TOKEN_BLD) > 1:
            self.__mergeTokenSequence(
                toks,
                [TOKEN_BLD, TOKEN_H4],
    ...
    def __splitJibun(self, toks, bldPos):
        """
        __splitJibun 메서드는 주어진 토큰 리스트에서 건물 번호를 분리합니다.

        필수 조건: BLD가 있고 BNG 없어야 함

        Args:
            toks (TokenList): 토큰 리스트 객체.
            bldPos (int): 건물 토큰의 위치.
        ...
        동작:
            - 주어진 위치의 토큰을 가져옵니다.
            - 토큰의 값이 숫자로 시작하는 경우, 해당 값을 기반으로 건물 번호를 분리합니다.
            - 특정 정규 표현식에 매칭되는 경우, 토큰을 분리하여 TOKEN_BLDNO로 설정합니다.
        """
        tkn = toks.get(bldPos)

        re_bunji_list = [
            self.re_bunjihead_n_n_bng,  # 1-2(번지)
            self.re_bunjihead_n_의_n,  # 1의2(번지)
            self.re_bunjihead_번지_호,  # 1번지2(호)
            self.re_bunjihead_n_n_호,  # 1013-6호
            self.re_bunjihead_n_호,  # 6호
            self.re_bunjihead_n_번지,  # 1번지
        ]

        for r in re_bunji_list:
            m = r.match(tkn.val)
            if m:
                toks.split(bldPos, m.span()[1], TOKEN_BNG, tkn.t)
                return

        # 숫자와 건물명 분리 (보수적으로)
        re_num_str = re.compile(r"^(\d+)(\S+)$")
        m = re_num_str.match(tkn.val)
        if m:
            num = m.group(1)
            rest = m.group(2)
            ...
            if rest.startswith(("호선", "동", "차", "단지", "호")):
                return

            # 88컨트리클럽, 21세기호프, 24시 빨래방, 25시주유소, 4.19기념도서관, 119안전센터파평지역대, 123다세대, 123빌라, 365맨션
            if num in ["88", "21", "24", "25", "419", "119", "123", "365"]:
                return

            if tkn.val.startswith("1급"):
                return  # 1급경남자동차정비

            toks.split(bldPos, len(num), TOKEN_BNG, tkn.t)
...
def load_list(filename: str):
    """{config.CODE_DATA_DIR}/{filename} 파일에서 행정구역 코드를 읽어옵니다."""
    codes = []
    try:
        with open(f"{config.CODE_DATA_DIR}/{filename}", "r", encoding="utf-8") as file:
            for line in file:
                line = line.strip()
                if line and not line.startswith("#"):  # 빈 줄과 주석 제외
                    codes.append(line)
    except FileNotFoundError:
        print(f"경고: {config.CODE_DATA_DIR}/{filename} 파일을 찾을 수 없습니다.")
